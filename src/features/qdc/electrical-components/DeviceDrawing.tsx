...
function DeviceDrawing({ device: d, width: w, height: h, mode }: Props) {
  ...
  if (d.type === 'comb-bus') {
    const bottom = d.combSide === 'bottom', barY = bottom ? h - 7 : 1;
    return <g>
  ...
  if (d.type === 'power-entry') return <g>
    <rect x="2" y="2" width={w - 4} height={h - 4} rx="6" fill="#e5e9e6" stroke="#829095" />
    <path d={`M${w / 2} 9 V${h - 5} M${w / 2 - 18} 15 H${w / 2 + 18} M${w / 2 - 14} 15 L${w / 2 - 18} 22 M${w / 2 + 14} 15 L${w / 2 + 18} 22`} fill="none" stroke="#485b63" strokeWidth="3" strokeLinecap="round" />
    <circle cx={w / 2 - 14} cy="15" r="3" fill="#d4d9d7" stroke="#485b63" /><circle cx={w / 2 + 14} cy="15" r="3" fill="#d4d9d7" stroke="#485b63" />
    <path d={`M${w / 2 - 4} ${h - 10} l5 -8 h-4 l6 -8 -2 7 h5 Z`} fill="#e2bd28" stroke="#836d17" strokeWidth=".6" />
  </g>;
  if (d.type === 'conduit-entry') return <g>
  ...
  return <g>
    <defs><linearGradient id={shellGradient} x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#fafbf8" /><stop offset=".52" stopColor="#e6e9e5" /><stop offset="1" stopColor="#cbd1ce" /></linearGradient></defs>
    <rect x="3" y="4" width={w - 4} height={h - 3} rx="4" fill="#233039" fillOpacity="0.17" />
    <path d={`M5 1 H${w - 5} Q${w - 2} 1 ${w - 2} 5 V${h - 5} Q${w - 2} ${h - 1} ${w - 6} ${h - 1} H5 Q1 ${h - 1} 1 ${h - 6} V5 Q1 1 5 1`} fill={terminal ? '#d4dce0' : `url(#${shellGradient})`} stroke="#9fa9aa" />
    <path d={`M5 5 H${w - 5} V${h - 7}`} fill="none" stroke="#fff" strokeWidth="2" />
    <path d={`M${w - 5} 8 V${h - 9}`} stroke="#aeb7b5" strokeWidth="2" opacity=".55" />
    <rect x="5" y="27" width={w - 10} height={h - 54} rx="2" fill={terminal ? '#7d9098' : '#e5e8e4'} stroke="#c2c9c8" />
    {switching ? <>
      {Array.from({ length: Math.max(d.poles, 1) }, (_, i) => {
        const pw = w / Math.max(d.poles, 1), px = pw * i;
        return <g key={i}>
          {i > 0 && <path d={`M${px} 4 V${h - 4}`} stroke="#bac3c4" />}
          <rect x={px + 8} y="62" width={pw - 16} height="30" rx="2" fill="#485356" />
          <path d={`M${px + 9} 68 H${px + pw - 9} V82 H${px + 9} Z`} fill={main ? '#2e4a58' : '#26353b'} stroke="#12252b" />
          <path d={`M${px + 10} 68 H${px + pw - 10}`} stroke="#718087" />
          <text x={px + pw / 2} y="77" textAnchor="middle" fill="#fff" fontSize="5.5">I ON</text>
          <rect x={px + pw / 2 - 5} y="87" width="10" height="3" rx="1" fill="#367a55" />
        </g>;
      })}
      <text x="8" y="39" fill={INK} fontSize={compact ? 9 : 10} fontWeight="800">{d.type === 'rcbo-2p' ? 'RCBO' : rcd ? 'IDR' : caption}</text>
